...
重构 `AgentService`，核心变化：
1. Agent CRUD 中移除内嵌 SubAgent 处理
2. 新增独立的 SubAgent 挂载/卸载方法
3. 新增循环引用检测
4. 新增"可挂载 Agent"查询

## 4.1 移除旧 SubAgent 内嵌逻辑
...
def _has_circular_reference(
    ...
    时间复杂度：O(N) where N = 总 agent_subagent 记录数（最坏情况）
    实际场景中 SubAgent 层级很浅（通常 2-3 层），性能不是问题。
    """
    # 从 child 出发，寻找其所有后代
    visited = set()
...
```
场景：已有 A → B → C，现在要将 A 挂载为 C 的 SubAgent

_has_circular_reference(parent=C, child=A)
  从 A 出发 BFS:
  A → 检查 A 的 SubAgent → 找到 B
  B → 检查 B 的 SubAgent → 找到 C
  C == parent_agent_id(C) → 返回 True ← 检测到环路！
```
...
def delete_agent(db: Session, agent_id: UUID) -> bool:
    ...
    注意：
    - CASCADE 会自动清除 agent_subagent 中的关联记录
    - 但软删除不触发 CASCADE，需要手动清理
    """
    agent = AgentService.get_agent(db, agent_id)
    if not agent:
        return False
    ...
    # 清除受影响的父 Agent 的缓存
    # (已通过 API 层调用 AgentFactory.clear_cache)
    return True
```
