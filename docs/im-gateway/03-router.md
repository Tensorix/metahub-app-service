...
**文件**: `app/router/v1/im_gateway.py`（新建）

## 职责

- WebSocket 端点：桥接服务连接入口
- REST 发送消息 API：通过桥接向 IM 平台发消息
- 状态查询 API：查看活跃连接
...
WebSocket 无法使用 HTTP Header，因此 token 通过 query params 传递，与 `agent_chat.py` 的 WebSocket 端点模式一致。支持 JWT 和 API Key 两种方式。

### 消息处理复用

`_handle_incoming_message` 将 WS 收到的消息直接转换为 `IMMessageWebhookRequest`，调用 `WebhookService.process_im_message()`。收消息逻辑完全复用，零重复。

### DB Session 管理

| 场景 | DB Session 来源 | 原因 |
|------|----------------|------|
| WS 主循环 | `SessionLocal()` in `im_gateway_ws` | WS 生命周期不走 FastAPI DI |
| 消息处理 | `SessionLocal()` in `_handle_incoming_message` | 独立事务，不阻塞 WS 循环 |
| REST 端点 | `Depends(get_db)` | 标准 FastAPI DI |
