# Step 3: Auto-Reply Service

## New File

**File**: `app/service/auto_reply.py`

## Responsibilities

1. 接收 session_id、incoming message 信息、user_id
2. 加载 Session 和关联的 Agent
3. 构建对话上下文（最近消息）
4. 调用 Agent 生成回复（非流式）
5. 将回复消息入库（role=self）
6. 通过 IM Bridge 发送到外部平台
...
### Per-Session Lock
- 使用 `asyncio.Lock` 防止同一会话的多条消息同时触发自动回复
- 如果锁被占用（上一条消息的自动回复还在进行），新消息直接跳过
- 避免消息风暴导致的重复回复

### Thread ID Strategy
- 使用 `auto_reply_{session_id}` 作为 thread_id
- Agent 在同一会话内保持对话连续性和记忆
- 与前端 AI 会话的 `topic_{topic_id}` 命名空间隔离

### Error Isolation
- 整个 process 方法在 try/except 中，任何失败仅记录日志
- 使用独立的 DB session，不影响 WebSocket 主循环的事务
- Bridge 发送失败也仅记录日志，消息已入库不会丢失

### Message Role
- 自动回复消息使用 `role=self`，与手动发送的消息一致
- 通过 `metadata_.auto_reply=True` 标记区分
- 前端可根据此标记展示不同的 UI 提示（可选，后续迭代）
