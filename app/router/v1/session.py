...
def update_session(session_id: UUID, data: SessionUpdate, db: Session = Depends(get_db)):
...
def mark_session_read(session_id: UUID, db: Session = Depends(get_db)):
...
def create_topic(session_id: UUID, data: TopicCreate, db: Session = Depends(get_db)):
...
def get_topics(session_id: UUID, db: Session = Depends(get_db)):
...
def delete_topic(topic_id: UUID, hard_delete: bool = Query(False), db: Session = Depends(get_db)):
...
def create_message(session_id: UUID, data: MessageCreate, db: Session = Depends(get_db)):
    if data.session_id != session_id:
        raise HTTPException(status_code=400, detail="session_id 不匹配")
...
def delete_message(message_id: UUID, hard_delete: bool = Query(False), db: Session = Depends(get_db)):
...
def delete_agent(agent_id: UUID, hard_delete: bool = Query(False), db: Session = Depends(get_db)):
