...
class ActivityService:
    ...
    @staticmethod
    def get_activity(db: Session, activity_id: UUID, include_deleted: bool = False) -> Optional[Activity]:
        """根据ID获取活动"""
    ...
    @staticmethod
    def get_activities(db: Session, query_params: ActivityListQuery) -> tuple[list[Activity], int]:
        """获取活动列表"""
    ...
    @staticmethod
    def update_activity(db: Session, activity_id: UUID, activity_data: ActivityUpdate) -> Optional[Activity]:
        """更新活动"""
    ...
    @staticmethod
    def delete_activity(db: Session, activity_id: UUID, soft_delete: bool = True) -> bool:
        """删除活动"""
        db_activity = ActivityService.get_activity(db, activity_id)
        if not db_activity:
            return False
    ...
    @staticmethod
    def restore_activity(db: Session, activity_id: UUID) -> Optional[Activity]:
        """恢复已删除的活动"""
        db_activity = ActivityService.get_activity(db, activity_id, include_deleted=True)
    ...
    @staticmethod
    def create_activity_from_event(db: Session, event_id: UUID, event_type: str, event_data: dict) -> Activity:
        """根据事件自动创建对应的活动"""
        # 根据事件类型生成活动名称
